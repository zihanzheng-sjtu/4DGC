...
def readColmapSceneInfo(path, images, eval, llffhold=8, testidx=[0], ply_name="points3D.ply"):
    new_path = re.sub(r'colmap_\d+', 'colmap_0', path)
    try:
    ...
    ply_path = os.path.join(new_path, "sparse/0/"+ply_name)
    bin_path = os.path.join(new_path, "sparse/0/points3D.bin")
    txt_path = os.path.join(new_path, "sparse/0/points3D.txt")
